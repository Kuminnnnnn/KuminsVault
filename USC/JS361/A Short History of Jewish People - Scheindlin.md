**BC 1000 - Cannan Istraelite kingdom**
1. Powerful Rivals
	1. Egypt
	2. Mesopotamia
		1. Assyrian
		2. Neo-Babylonian
---
# Origins
1. BC 2000 - Migration of **Semites & Amorites**
	1. **Seminomadic** people -> marginal group -> were called *Habiru/Apiru (a social status)*
	2. Amorites are parts of Semites
	3. ***These two are both the ancestors of Aramaeans and Israelites***
2. BC 1720 - Local Cannanites & Amorites took advantage of the weakness of Egyptian middle kingdom to established themselves in the delta
3. BC 1650 - Established their own Egyptian dynasty named Hyksos, lasting for 100 years
---
# Exodus & Back to Cannan
### I. Core Comparison Matrix

| **Dimension**         | **Biblical Tradition**                                                                                                                                              | **Modern History & Archaeology**                                                                                                                                                                                                            |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Origin & Identity** | Unified lineage: Direct descendants of Abraham, Isaac, and Jacob; experienced "family migration to Egypt $\rightarrow$ mass enslavement $\rightarrow$ mass Exodus." | Multi-origin fusion: Formed locally through the long-term convergence of diverse semi-nomadic groups (including those with _Apiru_ social status and local mountain nomads who never went to Egypt).                                        |
| **Driving Force**     | **Divine Providence**: All historical milestones (famines, slavery, Exodus, land acquisition) are driven by covenant plans and divine miracles.                     | **Sociopolitical Evolution**: Driven by Ancient Near Eastern mass migrations (e.g., West Semitic movements), fluctuating imperial power (e.g., Egyptian control over the Delta), and social mobility.                                       |
| **Entry into Canaan** | **Military Conquest**: Swift, sweeping campaigns led by Joshua to rapidly conquer and partition the entire land.                                                    | **Gradual Penetration & Indigenous Rise**: Archaeology confirms 13th-century BCE destruction of Canaanite cities, but the process was complex; Israelites steadily established roots in the **central highlands** rather than a blitzkrieg. |

### II. Section-by-Section Breakdown: Myth vs. History
#### 1. Ancestors & Descent into Egypt
- **Bible**: Abraham migrates from Mesopotamia to Canaan by divine command; famine forces Jacob's family (with Joseph as an Egyptian vizier) into Goshen (Nile Delta), where they multiply and are enslaved.
- **History / Archaeology**:
    - _Plausible Background_: Early 2nd millennium BCE saw mass migrations of West Semitic/Amorite semi-nomads; Levant-origin Hyksos ruled the Delta in the 17th–16th centuries BCE
    - _Historical Revision_: The "entire family in Egypt" is viewed as a tribal myth. Realistically, only a small subset of ancestors may have been in Egypt (or inspired by the _Apiru_ class phenomenon), while the vast majority of proto-Israelites never left Canaan.
#### 2. The Exodus & Pharaoh's Identity
- **Bible**: Moses performs miracles (Ten Plagues, parting the Red Sea) to lead all Israelites out of Egyptian bondage, receiving the Law at Mt. Sinai.
- **History / Archaeology**:
    - Mention of city-building (Pithom/Rameses) and the reign of Ramesses II (~1290–1224 BCE) places the timeframe in the 13th century BCE.
    - The **Merneptah Stele** (~1220 BCE) provides the earliest extra-biblical mention of "Israel" in Canaan.
    - _Key Distinction_: Not a mass migration of millions, but rather a small group of slaves/migrants with Egyptian ties who escaped/migrated back and integrated into Canaan, later mythologized as a national Exodus.
#### 3. Entering Canaan & Ethogenesis (Nation Formation)
- **Bible**: Israelites bypass Edom/Moab, defeat Amorites, cross the Jordan under Joshua, and rapidly conquer Canaanite lands (e.g., Jericho) for the 12 tribes.
- **History / Archaeology**:
    - _Core Difference (Single Bloodline vs. Multi-Origin)_: The Bible assumes all incoming settlers came from Egypt. Scholars agree the true nation was born when **"the Egypt-exodus group" merged with "indigenous, related tribal groups already living in Canaan"**.
    - _Conquest vs. Settlement_: The biblical text itself contains internal contradictions (rapid total conquest vs. slow, uncoordinated control). Archaeology supports the latter: central highland settlements grew organically in the late 13th century BCE, rather than capturing the coastal plains instantly.
___
# Periods of Judges
- 12 tribes
- Threats: Philistines
- Under pressure -> king required -> war talented Saul
---
# David & Solomon
- David controlled the region and made himself a center of a tiny empire
	- Defeated Ammon, Moab, Edom, Aramean states
- David had Solomon crowned
- Solomon developed economy
	- **Partners**
		- Egypt
		- Tyre
		- red sea
		- Arabia
		- Cilicians
		- Hittites
	- **Results**
		- Increased social burden
		- Requirement of bureaucracy
		- Social unsatisfaction
		- Aristocracy exploitation
---
# Fracture of the kingdom
### I. The Divided Monarchy (Post-Solomon)
- **The Split**: After Solomon's death, the united kingdom fractured due to heavy taxation and forced labor.
- **Northern Kingdom (Israel)**:
    - **Leader**: Jeroboam (elected by northern tribes, first capital at Shechem $\rightarrow$ Tirzah $\rightarrow$ Samaria).
    - **Religion**: Built alternative shrines at Dan and Bethel to prevent citizens from traveling to Jerusalem.
    - **Fate**: Politically unstable; lasted 200 years until destroyed by the **Assyrians in 721 BCE**. (Note: In this era, "Israel" refers specifically to the North).
- **Southern Kingdom (Judah)**:
    - **Leader**: Rehoboam (Solomon's son, of the Davidic dynasty).
    - **Capital**: Jerusalem (retained the official Temple and religious center).
    - **Fate**: More stable; lasted until destroyed by the **Babylonians in 587 BCE**.
### II. The Omride Dynasty & Religious Conflict
- **Omri & Ahab**: Northern kings who restored Israelite power through Phoenician alliances and intermarriage (Ahab married **Jezebel** of Tyre).
- **The Clash of Faiths**:
    - Jezebel aggressively promoted foreign pagan worship (Baal) and persecuted Yahweh's followers.
    - This sparked fierce opposition from prophets like **Elijah and Elisha**.
- **The Jehu Coup (842 BCE)**: General Jehu led a bloody revolt, executing the royal family (throwing Jezebel out a window) and purging Baal worship in the North.
### III. Parallel Turmoil in Judah
- **Athaliah's Reign**: An Omride princess married into Judah's royal line, seized power after her son's death, and introduced Baal worship to the South.
- **The Restoration (837 BCE)**: The High Priest hid a surviving young prince, orchestrated a coup, executed Athaliah, and restored traditional Yahwistic worship in Jerusalem.
---
# Turmoil
#### 1. The Fall of the Northern Kingdom (721 BCE)

- **Brief Revival**: Jeroboam II (Israel) and Uzziah (Judah) experienced a golden age of expansion.
    
- **Assyrian Conquest**: Tiglath-Pileser III and his successors crushed the region.
    
- **The End of Israel**: Samaria fell in **721 BCE** to Sargon II. The population was exiled/mixed, giving rise to the legend of the "Ten Lost Tribes."
    

#### 2. Judah's Survival & Josiah's Reform (Late 7th Century BCE)

- **Hezekiah**: Fortified Jerusalem and built the Siloam Tunnel to survive the Assyrian siege (701 BCE).
    
- **Josiah's Reform**: Capitalizing on Assyria's decline, Josiah centralized all worship in Jerusalem and purged foreign idols. He died fighting Egypt in 609 BCE, plunging Judah into foreign control.
    

#### 3. The Fall of Judah & The Babylonian Exile (587 BCE)

- **Babylonian Supremacy**: Nebuchadnezzar crushed Egyptian influence and turned his sights on rebellious Judah.
    
- **Destruction of Jerusalem**: After a prolonged siege, Jerusalem and its Temple were burned to the ground in **587 BCE**. The elite were exiled to Babylon.
    
- **Legacy**: In exile, the survivors preserved their faith, developed structured religious institutions, solidified **monotheism**, and fostered the hope of a **Messianic restoration**.